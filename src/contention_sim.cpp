...
void ContentionSim::enqueue(TimingEvent* ev, uint64_t cycle) {
    ...
    // tby: modify limit by 100
    assert_msg(cycle < lastLimit+10*zinfo->phaseLength+100000000, "Queued event too far into the future, cycle %ld lastLimit %ld", cycle, lastLimit);
