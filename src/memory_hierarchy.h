...
struct MemReq {
    ...
    enum Flag {
        ...
        // tby: added for PimBridge
        TRANSFER_REGION  = (1<<6), // Transfer region access. This is generated by toMemEndpoints and only for memory (PimBankMemory) 
    };
    uint32_t flags;
...
class MemObject : public GlobAlloc {
    public:
        ...
        virtual uint64_t access(MemReq& req, bool isCritical, uint32_t data_size) { assert(false); }
